<template>
  <div class="download-center">
    <el-card shadow="hover">
      <div class="header">
        <h1>
          <el-icon><el-icon-download /></el-icon> 基因组数据下载中心
        </h1>
...
<script>
import { Download as ElIconDownload } from '@element-plus/icons'
    ...
    async loadFiles() {
      try {
        ...
        this.fileList = response.data.files.map((file) => ({
          ...file,
          path: decodeURIComponent(file.path).replace(/\\/g, '/'), // 确保统一使用正斜杠
        }))
